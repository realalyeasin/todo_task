...
class ToDoController extends GetxController {
  ...
  Future<void> addToD(String task, bool done, String id) async {
    await FirebaseFirestore.instance.collection('todos').doc(
        (id != '' ? id : null)).set({
      'task': task,
      'isDone': done
  ...
  Future<void> getData() async {
    try {
      QuerySnapshot _taskSnap = await FirebaseFirestore.instance.collection(
          'todos').orderBy('task').get();
      todoList.clear();
  ...
  Future queryData(String q) async {
    return FirebaseFirestore.instance.collection('todos').where('task',isGreaterThanOrEqualTo: q).get();
  }
}
